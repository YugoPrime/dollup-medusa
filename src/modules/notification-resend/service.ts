...
import { isSendableEmail } from "../../lib/sendable-email"
...
// Moved to src/lib/sendable-email.ts so the notification-health job can share
// the exact same rule without importing this file (and with it `resend`, React
// and every .tsx template). Re-exported here so existing call sites — the
// preorder subscriber, deposit cleanup job and mark-paid route — keep working.
export { isSendableEmail }
